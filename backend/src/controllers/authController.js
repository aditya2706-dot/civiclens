...
const loginUser = async (req, res) => {
    const { email, password } = req.body; // 'email' is used as a generic identifier from frontend

    try {
        // Multi-Identifier Login: Support Email, Phone, or Username
        ...
        if (user && (await user.matchPassword(password))) {
            ...
            console.warn(`[AUTH] Failed login attempt for identifier: ${email}`);
            res.status(401).json({ message: 'Invalid credentials. Please check your username/email and password.' });
        }
    } catch (error) {
